...
@parser_classes([MultiPartParser, FormParser])  # ✅ acepta archivos
def productos_list_create(request):
    # GET: cliente ve SOLO disponibles
    ...
    producto = serializer.save(
        id_usuario=request.user,  # ✅ aquí se asigna el usuario
...
@permission_classes([IsAuthenticated])
@parser_classes([MultiPartParser, FormParser])  # ✅ por si editas imagen también
def productos_detail(request, pk):
    ...
    # GET: admin puede ver todo, cliente solo si está Disponible
    if request.method == "GET":
        if es_admin(request.user) or (
            prod.id_estado and (prod.id_estado.descripcion_estado or "").lower() == "disponible"
        ):
