...
def es_gestor_reservas(user):
    """
    Permisos tipo 'admin' SOLO para RESERVAS/FACTURAS:
    Admin (1) y Recepcionista (3)
    """
    return (
        getattr(user, "id_rol", None)
        and user.id_rol.id_rol in (ROLE_ADMIN, ROLE_RECEPCIONISTA)
    )
...
def crear_reserva_desde_carrito(usuario, items, observaciones=""):
    ...
    for p in productos:
        ...
        if cant > stock:
            raise ValueError(
                f"Stock insuficiente para '{p.nombre_producto}'. Disponible: {stock}"
            )
...
def listar_reservas_usuario(usuario):
    return (
        Reserva.objects.filter(id_usuario=usuario)
...
def cancelar_reserva(usuario, reserva: Reserva):
    estado_actual = (
        (reserva.id_estado.descripcion_estado or "").strip().lower()
        if reserva.id_estado
        else "pendiente"
    )
    if estado_actual != "pendiente":
        raise ValueError("Solo puedes cancelar reservas en estado Pendiente.")

    # ✅ Dueño o Gestor (Admin/Recepcionista)
    if (not es_gestor_reservas(usuario)) and (reserva.id_usuario_id != usuario.id_usuario):
...
def actualizar_estado_reserva(usuario, reserva: Reserva, *, estado_nombre=None, id_estado=None):
    # ✅ Admin/Recepcionista pueden marcar entregado/otro estado
    if not es_gestor_reservas(usuario):
        raise PermissionError("No autorizado.")
